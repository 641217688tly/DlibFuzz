...
def fetch_issues(repo_owner: str, repo_name: str, label: str='bug', num_results: int=100) -> list[dict]:
    ...
    while len(issues) < num_results:
        url = f'https://github.com/{repo_owner}/{repo_name}/issues?q=is%3Aissue+is%3Aopen+label%3A{label}&page={page}&per_page=30'
        response = requests.get(url)
        if response.status_code != 200:
            raise Exception(f"Failed to fetch issues: {response.status_code}")

        soup = BeautifulSoup(response.text, 'html.parser')
        issue_rows = soup.find_all('div', {'class': 'js-issue-row'})
        if not issue_rows:
            break  # Exit if no more issues are found

        for issue in issue_rows:
            title = issue.find('a', {'data-hovercard-type': 'issue'}).text.strip()
            issue_url = 'https://github.com' + issue.find('a', {'data-hovercard-type': 'issue'})['href']
            content = fetch_issue_content(issue_url)
            issues.append({'title': title, 'url': issue_url, 'content': content})
