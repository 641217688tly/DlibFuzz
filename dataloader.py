...
def attach_history_errors(db_session, dir_path, lib):
    ...
    for count, filename in enumerate(json_files, start=1):  # start=1表示从1开始计数
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            existing_error = db_session.query(APIHistoryError).filter_by(title=title, code=code, description=description).first()
            if existing_error:
...
if __name__ == '__main__':
    ...
    # add_apis_from_json(session, 'cluster/apis/jittor/jittor_apis.json', 'Jittor', "1.3.9.10")
    ...
    # added_jittor_errors_num = attach_history_errors(session, jittor_dir, 'Jittor')
