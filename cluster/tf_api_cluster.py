...
class Clusterer:
    ...
    def initialize_message(self):  # 构建clusterer的初始提词并返回对话消息
        ...
        example2 = """json
        {   
            // Output an empty dictionary when no combination output from the Pytorch API or JAX API has the same value as the TensorFlow API
            "Pytorch" : {}, 
            "Tensorflow" : {
        ...
        """
        clusterer_prompt = f"""
        Which function apis or combinations of function api calls in PyTorch (v{self.torch_ver}) and JAX (v{self.jax_ver}) have the exact same functionality as {self.api.full_name} in TensorFlow (v{self.tf_ver})?
