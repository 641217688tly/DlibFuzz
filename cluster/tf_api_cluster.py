...
class TensorflowClusterer:
    ...
    def initialize_message(self):  # 构建clusterer的初始提词并返回对话消息
        clusterer_prompt = f"""
Objective:
Identify equivalent or identical API functions or combinations of functions in PyTorch (v{self.torch_ver}) and JAX (v{self.jax_ver}) that perform the same tasks as the function {self.api.full_name} in Tensorflow (v{self.tf_ver}).
...
"""
    ...
    def save_cluster(self, json_data):
        ...
        try:
            ...
            # 2. 如果torch_apis_combination_objects和jax_apis_combination_objects有一个不为空则创建Cluster对象:
            if torch_apis_combination_objects or jax_apis_combination_objects:
                new_cluster = Cluster(
                    energy=5,
                )
...
def run_randomly():  # 随机挑选未聚类的TensorflowAPI进行聚类
    # 创建数据库连接
    session = get_session()
    openai_client = get_openai_client()

    # 对未聚类的TensorflowAPI进行聚类
    uncluttered_torch_apis = session.query(TensorflowAPI).filter_by(is_clustered=False).all()
    while uncluttered_torch_apis:
        print("----------------------------------------------------------------------------------")
        # 随机选择一个未聚类的TensorflowAPI
        uncluttered_torch_api = random.choice(uncluttered_torch_apis)
        clusterer = TensorflowClusterer(uncluttered_torch_api, session, openai_client)
        clusterer.cluster_api()

        uncluttered_torch_apis = session.query(TensorflowAPI).filter_by(is_clustered=False).all()
        total_apis_num = session.query(TensorflowAPI).count()
        unclustered_torch_apis_num = len(uncluttered_torch_apis)
        print(f"Unclustered / Total: {unclustered_torch_apis_num} / {total_apis_num}")


def run_linearly():  # 线性地对未聚类的TensorflowAPI进行聚类
    # 创建数据库连接
    session = get_session()
    openai_client = get_openai_client()
