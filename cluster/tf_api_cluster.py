...
class Clusterer:
    ...
    def initialize_message(self):  # 构建clusterer的初始提词并返回对话消息
        clusterer_prompt = f"""
Task:
Identify equivalent or identical API functions in PyTorch (ver{self.torch_ver}) and JAX (ver{self.jax_ver}) that perform the same tasks as the function ({self.api.signature}) in Tensorflow (ver{self.tf_ver}).
...
"""
    ...
    def save_cluster(self, json_data):
        ...
        try:
            ...
            # 2. 如果torch_apis_objects和jax_apis_objects有一个不为空, 则创建Cluster对象
            if torch_apis_objects or jax_apis_objects:
                new_cluster = Cluster(
                    energy=5,
                    base='Tensorflow',  # 设置基础库
                )
