...
class Clusterer:
    ...
    def initialize_message(self):  # 构建clusterer的初始提词并返回对话消息
        clusterer_prompt = f"""
Objective:
Identify equivalent or identical API functions in TensorFlow (v{self.tf_ver}) and JAX (v{self.jax_ver}) that perform the same tasks as the function {self.api.full_name} in Pytorch (v{self.torch_ver}).

Steps:
1.Identify the Functionality: First, understand the functionality of {self.api.full_name} in Pytorch (v{self.torch_ver}).
2.Search for Equivalents: Then, find API functions in TensorFlow (v{self.tf_ver}) and JAX (v{self.jax_ver}) that match this functionality.
...
"""
    ...
    def validate_apis(self, response):  # 验证LLM返回的JSON中所有的API是否有效或真实存在
        ...
        try:
            ...
            for dl_lib, apis in json_data.items():  # 逐个访问Pytorch, Tensorflow和JAX下的APIs
    ...
    def supplement_apis(self, apis, api_class):  # 将LLM生成的JAX/Pytorch/Tensorflow的不在数据库中的API添加到数据库中
        """
        以下列数据为例:
        apis = "Tensorflow" : {
            "1" : "tensorflow.keras.losses.CategoricalCrossentropy",
    ...
    def save_cluster(self, json_data):
        ...
        try:
            ...
            # 2. 如果tf_apis_objects和jax_apis_objects有一个不为空, 则创建Cluster对象
            if tf_apis_objects or jax_apis_objects:
                new_cluster = Cluster(
                    energy=5,
                    base='Pytorch',  # 设置基础库
                )
...
def run_linearly():  # 线性地对未聚类的PytorchAPI进行聚类
    # 创建数据库连接
    session = get_session()
    openai_client = get_openai_client()
